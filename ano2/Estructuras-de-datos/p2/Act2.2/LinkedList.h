...
// metodo para agregar todos los datos de un linked list al final de otro
// O(n^2) porque addLast() es O(n), y esta dentro de otro loop
template <class T>
void LinkedList<T>::operator += (const LinkedList& list1){
    Node<T> *curr2 = list1.head; //se guarda el head de la lista que se va a agregar
    for(int i=0;i<list1.getSize();i++){
        this->addLast(curr2->getData()); // agrega todos los datos de la lista comparada a
        curr2 = curr2->getNext(); // la lista que se le esta agregando
    }
}

// Es O(n^2) porque el metodo de addLast() es O(n)
template <class T>
void LinkedList<T>::operator = (const LinkedList& list1){
    this->deleteAll(); // como se va a igualar a la otra lista, se le borran todos sus datos
    Node<T> *curr2 = list1.head; //se guarda el head de la lista que se va a agregar
    for(int i=0;i<list1.getSize();i++){
        this->addLast(curr2->getData()); // se le guardan todos los datos a la lista
        curr2 = curr2->getNext(); // que se vacio
    }
}

// Otra vez O(n^2) por el metodo addLast()
template <class T>
LinkedList<T>::LinkedList(const LinkedList& list1){
    head = nullptr;
    size = 0;
    Node<T> *curr2 = list1.head; //se guarda el head de la lista que se va a agregar
    for(int i=0;i<list1.getSize();i++){
