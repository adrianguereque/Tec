...
// Descripcion: Implementamos las operaciones MaxWidth para conocer la maxima anchura del BST, nearestRelative para conocer 
// en ancestro mas cercano de dos datos y operator== que compara dos BST. Tambien se hara una copia constructor del BST que se esta construyendo

// Ultima fecha de modificacion: 15 Oct 2023
...
class BST {
	public:
		...
		// Actividad 3.2
		int maxWidth() const; // O(n)
		int nearestRelative(int num1, int num2) const; // 0(n)
		BST(const BST &bst);
	...
	private:
		...
		NodeT *tree2(NodeT *node) const;
		bool isEqual(NodeT *node1, NodeT *node2) const; // O(n)
...
bool BST::isEqual(NodeT *node1, NodeT *node2) const {
	if (node1 == nullptr || node2 == nullptr) {
		return true;
	}

	if (node1 != nullptr || node2 != nullptr) {
		return (node1->getData() == node2->getData()) || isEqual(node1->getLeft(), node2->getLeft()) || isEqual(node1->getRight(), node2->getRight());
	}
	return false;
}
...
NodeT *BST::tree2(NodeT *node) const {
	if (node == nullptr) {
		return nullptr;
	}
	NodeT *newNode = new NodeT(node->getData());
	newNode->setLeft(tree2(node->getLeft()));
	newNode->setRight(tree2(node->getRight()));
	return newNode;
}
